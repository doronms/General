...
#else
int fzetta_fmc_iic_init(void __iomem* BaseAddress, u8 Dev_ID)
#endif
{
	int Status;
	/*
...
int fzetta_fmc_iic_xbar_register_write(void __iomem* BaseAddress,u8 RegAddr, u8 RegData)
{
   int Status;
   u8 byte_count = 2;
   static u8 WriteBuffer[2];
    ...
    Status = XIic_Send(BaseAddress, XBAR_IIC_WRITE_ADDR, WriteBuffer, byte_count, XIIC_STOP);
...
u8 fzetta_fmc_iic_xbar_register_read(void __iomem* BaseAddress, u8 RegAddr){
		return 0;
	   ...
	   Status = XIic_DynInit(BaseAddress);
	   if (Status != XST_SUCCESS) {
	             return XST_FAILURE;
	   }
	  ...
	  /*
	    ...
	    */
	   while (((Status = XIic_ReadReg(BaseAddress,
	                           XIIC_SR_REG_OFFSET)) &
	   ...
	   XIic_DynSend7BitAddress(BaseAddress, XBAR_IIC_WRITE_ADDR, XIIC_WRITE_OPERATION);
	   XIic_WriteReg(BaseAddress,  XIIC_DTR_REG_OFFSET, WriteBuffer[0]);
		CntlReg = XIic_ReadReg(BaseAddress,  XIIC_CR_REG_OFFSET);
		CntlReg |= XIIC_CR_NO_ACK_MASK;
		XIic_WriteReg(BaseAddress,  XIIC_CR_REG_OFFSET, CntlReg);
	   XIic_DynSend7BitAddress(BaseAddress,XBAR_IIC_WRITE_ADDR, XIIC_READ_OPERATION);
	   XIic_DynSendStop(BaseAddress, 1);
	   ReadBuffer[0] = XIic_ReadReg(BaseAddress, XIIC_DRR_REG_OFFSET);
