...
int fzetta_fmc_ip_init(void __iomem*pGpio_add, void __iomem*pIic_addr, void __iomem*pSpi_addr)
{
	int Status = XST_SUCCESS;

#if 1
	Status |= fzetta_fmc_gpio_init(pGpio_add, fzetta_fmc_table.gpio_dev_id);
	Status |= fzetta_fmc_iic_init(pIic_addr, fzetta_fmc_table.iic_dev_id);
	Status |= fzetta_fmc_spi_init(pSpi_addr, fzetta_fmc_table.spi_dev_id);
#elif defined(SDT)
 ...
 *
 ******************************************************************************/
int fzetta_fmc_register_write(void __iomem* BaseAddress,fzetta_dev_type *Dev, u8 *Channel, spi_slave_sel *Slave_Sel, u8 *RegAddr, u8 *RegData)
{
	int Status = XST_SUCCESS;

	//if (*Dev == IIC_Dev)
	//	printk("fzetta_fmc_dev_init Dev is IIC\n");
	//else if (*Dev == SPI_Dev)
	//	printk("fzetta_fmc_dev_init Dev is SPI\n");
	//else
	//	printk("fzetta_fmc_dev_init Dev is UNKNOWND - %d\n", *Dev);


	if (*Dev == IIC_Dev) {
		Status |= fzetta_fmc_iic_xbar_register_write(BaseAddress,*RegAddr, *RegData);
	}
	else if (*Dev == SPI_Dev) {
		Status |= fzetta_fmc_spi_channel_select(*Channel);
		Status |= fzetta_fmc_spi_devices_register_write(BaseAddress,*Slave_Sel, *RegAddr, *RegData);
	}
	else {
		return XST_FAILURE;
	}
...
u8 fzetta_fmc_register_read(void __iomem* BaseAddres,fzetta_dev_type Dev, u8 Channel, spi_slave_sel Slave_Sel, u8 RegAddr)
{
	int Status = XST_SUCCESS;
	u8 RegData;

	if (Dev == IIC_Dev) {
		RegData = fzetta_fmc_iic_xbar_register_read(BaseAddres,RegAddr);
	}
	else if (Dev == SPI_Dev) {
		Status |= fzetta_fmc_spi_channel_select(Channel);
		RegData = fzetta_fmc_spi_devices_register_read(BaseAddres,Slave_Sel, RegAddr);
	}
	else
		return XST_FAILURE;
 ...
 *
 ******************************************************************************/
int fzetta_fmc_dev_init(void __iomem* IicBaseAddres, void __iomem* SpiBaseAddres,fzetta_fmc_reg *fzetta_fmc_reg_init)
{
	int i;
	int Status = XST_SUCCESS;
    ...
    for (i = 0; i < sizeof(fzetta_fmc_reg_init->reglist)/sizeof(fzetta_fmc_reglist); i++) {
		...
		void __iomem* BaseAddres = fzetta_fmc_reg_init->reglist[i].dev == IIC_Dev ? IicBaseAddres : SpiBaseAddres;

		Status |= fzetta_fmc_register_write(BaseAddres,
											&fzetta_fmc_reg_init->reglist[i].dev, &fzetta_fmc_reg_init->reglist[i].channel, 
    ...
    printk("General Fidus FMC Initialization Done\n\r");
	return XST_SUCCESS;
...
int fzetta_fmc_dev_errata_init(void __iomem* IicBaseAddres, void __iomem* SpiBaseAddres)
{
	int i;
	u8 ch, dev_id;
	int Status = XST_SUCCESS;
	...
	for (ch=0; ch<4; ch++) {
		...
		dev_id = fzetta_fmc_register_read(SpiBaseAddres,SPI_Dev, ch, SPI_RCLKR, DEV_ID_REG);
		switch (dev_id) {
			case 0x80: //M23145G-11P
				for (i = 0; i < sizeof(rclkr_errata_id_80)/sizeof(fzetta_fmc_reglist); i++) {
					Status |= fzetta_fmc_register_write(ch == IIC_Dev ? IicBaseAddres : SpiBaseAddres,
														&rclkr_errata_id_80[i].dev, &ch, 
			...
			default:
				printk("Channel %d No Errata for Reclocker Dev ID (0x%02x)\r\n",ch, dev_id);
		}

		//Actual Register Initiliazation Routine
	    if (Status == XST_SUCCESS) {
			printk("Channel %d Reclocker Errata Init Done (Dev ID 0x%02x)\n\r", ch, dev_id);
		}
	    ...
	    dev_id = fzetta_fmc_register_read(IicBaseAddres,SPI_Dev, ch, SPI_RCVR, DEV_ID_REG);
		switch (dev_id) {
			case 0x01: //M23544G-12P
				for (i = 0; i < sizeof(rcvr_errata_id_01)/sizeof(fzetta_fmc_reglist); i++) {
					Status |= fzetta_fmc_register_write(ch == IIC_Dev ? IicBaseAddres : SpiBaseAddres,
														&rcvr_errata_id_01[i].dev, &ch, 
			...
			default:
				printk("Channel %d No Errata for Receiver Dev ID (0x%02x)\r\n", ch, dev_id);
		}
	    if (Status == XST_SUCCESS) {
			printk("Channel %d Receiver Errata Init Done (Dev ID 0x%02x)\n\r", ch, dev_id);
		}
...
int fzetta_fmc_init(void __iomem* pGpio_add, void __iomem* pIic_addr, void __iomem* pSpi_addr)
{
   int Status = XST_SUCCESS;

   fzetta_fmc_table_init();
   Status |= fzetta_fmc_ip_init(pGpio_add, pIic_addr, pSpi_addr);
   Status |= fzetta_fmc_dev_init(pIic_addr, pSpi_addr,&fzetta_fmc_table);
   Status |= fzetta_fmc_dev_errata_init(pIic_addr, pSpi_addr);
