...
#else
int fzetta_fmc_spi_init(void __iomem * BaseAddress, u8 Dev_ID)
{
#endif
	int Status;
	/*
	...
	Status = XSpi_CfgInitialize(&fzetta_fmc_Spi, fzetta_fmc_Spi_ConfigPtr, BaseAddress);
...
int fzetta_fmc_spi_devices_register_write(void __iomem* BaseAddress, spi_slave_sel Slave_Sel, u8 RegAddr, u8 RegData)
{
   int match;
   int Status;
...
u8 fzetta_fmc_spi_devices_register_read(void __iomem* BaseAddres, u32 Slave_Sel, u8 RegAddr)
{
   int Status;
   static u8 ReadBuffer[2];
   static u8 WriteBuffer[2];
