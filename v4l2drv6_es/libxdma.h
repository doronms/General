...
struct xdma_dev {
	...
	int mum_of_vid_in_ch; /*shemie */
	int mum_of_vid_out_ch;/* shemie */
	
	/*struct video_output_video_device* pvid_out; shemie*/
	
	int mum_of_sdi_in_ch;
	int mum_of_sdi_out_ch;
	int mum_of_analog_in_ch;
	int mum_of_analog_out_ch;
	struct video_in_channle* psdi_in;
	struct video_output_video_device* psdi_out;
	struct video_in_channle* panalog_in;
	struct video_output_video_device* panalog_out;
...
ssize_t xdma_xfer_submit_nowait(void* cb_hndl, void* dev_hndl, int channel, bool write, u64 ep_addr, struct sg_table* sgt, bool dma_mapped, int timeout_ms);
