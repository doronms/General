 ...
 *****************************************************************************/
int Si5324_DoSettings(void *IICBaseAddress, u8 IICAddress,
	...
	for (i = 0; i < NumRegs; i++) {
		result = XIic_Send((UINTPTR)IICBaseAddress, IICAddress,
					BufPtr + (i << 1), 2,
					XIIC_STOP);
		if (result != 2) {
			if (SI5324_DEBUG) {
 ...
 *****************************************************************************/
int Si5324_Reset(void *IICBaseAddress, u8 IICAddress) {
	int result;
	u8 buf[2];
 ...
 * @return   SI5324_SUCCESS for success, SI5324_ERR_IIC for IIC access failure.
 *****************************************************************************/
int Si5324_Init(void *IICBaseAddress, u8 IICAddress) {
	int result;
 ...
 *****************************************************************************/
int Si5324_SetClock(void * IICBaseAddress, u8 IICAddress, u8 ClkSrc,
...
int Si570_DoSettings(void *IICBaseAddress, u8 IICAddress1,
		u8 *BufPtr, int NumRegs) {
	int result;
	int i;
 ...
 */
int Si570_SetClock(void *IICBaseAddress, u8 IICAddress1, u32 RxRefClk) {

	int result = 0;

	u8  buf[20*2]; /* Need to set 8 registers */
	int i;
