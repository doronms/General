...
static void user_interrupts_enable(struct xdma_dev *xdev, u32 mask)
{
	...
	//pr_info("user_interrupts_enable mask=%08x reg offset=%p DMA_OFS_INT_CNTL=%08x\n",
	//		mask, &reg->user_int_enable_w1s, XDMA_OFS_INT_CTRL);
...
static irqreturn_t user_irq_service(int irq, struct xdma_user_irq *user_irq)
{
	...
	//pr_info("user_irq_service user_irq=%p handler=%p\n",user_irq,user_irq->handler);
...
static irqreturn_t xdma_isr(int irq, void *dev_id)
{
	...
	if (user_irq) {
		...
		for (; user < max && user_irq; user++, mask <<= 1) {
			if (user_irq & mask) {
				user_irq &= ~mask;
				//pr_info("calling user_irq_service user = %d\n",user);	
...
static int is_config_bar(struct xdma_dev *xdev, int idx)
{
	...
	printk("is_congig_bar %px %px \n", xdev->bar[idx], irq_regs);
...
static int map_bars(struct xdma_dev *xdev, struct pci_dev *dev)
{
	...
	rv = map_single_bar(xdev, dev, 0);
	if (rv <= 0) {
		pr_info("%s, map config bar %d failed, %d.\n",
			dev_name(&dev->dev), 0, rv);
		return -EINVAL;
	}
...
static int probe_engines(struct xdma_dev *xdev)
{
	...
	for (i = 0; i < xdev->c2h_channel_max; i++) {
		...
		pr_info("prob endignes DMA_FROM_DEVICE %d %d\n",i,rv);
		if (rv)
			break;
	}
		       ...
		       int *h2c_channel_max, int *c2h_channel_max)
{
	...
	pr_info("----------- xdma_device_open calling  enable_msi_msix --------------- \n");
	...
	*c2h_channel_max = xdev->c2h_channel_max;
	...
	pr_info("----------- xdma_device_open return ok--------------- \n");
	return (void *)xdev;
			   ...
			   irq_handler_t handler, void *dev)
{
	...
	pr_info("xdma_user_isr_register >>>> mask %x\n",mask);
	...
	for (i = 0; i < xdev->user_max && mask; i++) {
		...
		mask &= ~bit;
		pr_info("xdma_user_isr_register setting handler for irq %d user_irq add: %p\n",i, &xdev->user_irq[i]);
		xdev->user_irq[i].handler = handler;
		xdev->user_irq[i].dev = dev; //ido
	}
