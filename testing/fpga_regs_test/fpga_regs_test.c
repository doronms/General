...
int fd;
/*
unsigned long long rdregsbuf[RD_REGS_NO];
unsigned long long wrregsbuf[WR_REGS_NO];
unsigned long long rdwrregsbuf[WR_REGS_NO];
*/
unsigned int rdregsbuf[RD_REGS_NO];
unsigned int wrregsbuf[WR_REGS_NO];
unsigned int rdwrregsbuf[WR_REGS_NO];
...
int main(int argc, char* argv[])
{
	...
	if (argc < 4)
	{
		printf("usage fpga_regs_test -r <reg_offset> <reg_count> to read registers.\n");
		printf("usage fpga_regs_test -w <reg_offset> <reg_value[HEX] > to write one eregisters.\n");
		exit (-1);
	}
	...
	fd = open("/dev/fpga_regs", O_RDWR);
	if (fd < 0)
	{
		printf("error, cannot open /dev/fpga_regs\n");
		return -1;
	}
	...
	if (mode == 0)	// read registers
	{
		...
		result = ioctl(fd, GET_FPGA_REGS, &regs);
	...
	else		// write registers
	{
		...
		result = ioctl(fd, SET_FPGA_REGS, &regs);
			if (result < 0)
		{
